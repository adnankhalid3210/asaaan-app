...
export class ControlsComponent implements OnInit {
  ...
  constructor(public appService: AppService, public snackBar: MatSnackBar, public auth: AuthService, public router: Router) { }
  ...
  public hire(person) {
    ...
    this.auth.logInState.subscribe(isLogin => {
      if (isLogin) {
        let user = JSON.parse(localStorage.getItem('user'))
        console.log(person.user.id)
        if (person.user.id !== user.id) {
          localStorage.setItem('sp', JSON.stringify(person));
          this.router.navigate(['/checkout'])

        }
        else {
          this.snackBar.open('You can not hire yourself!', '×', { panelClass: 'danger', verticalPosition: 'top', duration: 3000 });
        }
      }
      else {
        localStorage.setItem('hiring', 'true');
        localStorage.setItem('sp', JSON.stringify(person));
